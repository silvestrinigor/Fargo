...
public sealed class PartitionDeleteCommandHandlerTests
{
    ...
    private readonly PartitionDeleteCommandHandler handler;

    public PartitionDeleteCommandHandlerTests()
    {
        partitionService = new PartitionService(partitionRepository);

        handler = new PartitionDeleteCommandHandler(
            partitionService,
            partitionRepository,
            userRepository,
            currentUser,
    ...
    public async Task Handle_Should_ThrowUnauthorizedAccessFargoApplicationException_When_ActorIsNotFound()
    {
        ...
        partitionRepository.DidNotReceive().Remove(Arg.Any<Partition>());
    ...
    {
        // Arrange
        var actor = CreateUserWithPermission(ActionType.DeletePartition);
    ...
    [Fact]
    public async Task Handle_Should_RemovePartitionAndSaveChanges_When_ActorHasPermissionAndAccess()
    {
        // Arrange
        var actor = CreateUserWithPermission(ActionType.DeletePartition);
        var partition = CreatePartition();
        ...
        // Assert
        partitionRepository.Received(1).Remove(partition);
    ...
    private static PartitionDeleteCommand CreateCommand(Guid partitionGuid)
    {
        return new PartitionDeleteCommand(partitionGuid);
    }
